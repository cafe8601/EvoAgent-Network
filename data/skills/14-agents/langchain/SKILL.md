...
dependencies: [langchain, langchain-core, langchain-openai, langchain-anthropic, langgraph-checkpoint-postgres]
---
...
### 5. LangGraph PostgreSQL Checkpoints (Production)

In-memory memory is lost on restart. For production, use PostgreSQL checkpoints:
...
# Initialize checkpointer (run once)
...
with pool.connection() as conn:
    ...
    # Each thread_id = separate conversation (multi-tenant)
...
- **Multi-tenant**: `thread_id` isolates user conversations
- **Scalability**: Connection pooling handles concurrent users
- **Async support**: Use `AsyncPostgresSaver` for async workflows
